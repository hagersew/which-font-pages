...
import { StoreTextLinks } from '@/components/ui/StoreTextLinks';
      ...
      <div className="relative z-[2] mx-auto grid max-w-7xl items-center gap-12 px-4 md:px-6 lg:grid-cols-[1.05fr_0.95fr]">
        <div className="flex flex-col gap-6">
          {items.map((item) => (
            ...
            >
              {item.key === 'badge' && (
                <div className="inline-flex w-fit flex-wrap items-center gap-x-2 gap-y-1 rounded-full border border-brand-500/30 bg-brand-600/10 px-3 py-1 text-xs font-medium text-brand-400">
                  <span className="inline-flex items-center gap-2">
                    <span className="h-1.5 w-1.5 rounded-full bg-brand-400 animate-pulse" />
                    Available on
                  </span>
                  <StoreTextLinks linkClassName="text-brand-400" />
                </div>
              )}
              {item.key === 'heading' && (
